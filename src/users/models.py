...
class Profile(models.Model):
    ...
    work_period = models.CharField(
        _('период работы во время чемпионата'), choices=Period.CHOICES,
        max_length=8
    )
    work_shift = models.CharField(
        _('смена работы во время чемпионата'), choices=Shift.CHOICES,
        max_length=8
    )
    work_period_new = models.ForeignKey(
        Period, verbose_name=_('период работы во время чемпионата'), null=True

    )
    work_shift_new = models.ForeignKey(
        Shift, verbose_name=_('смена работы во время чемпионата'), null=True
    )
